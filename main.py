...
parser.add_argument("-l", "--load", action="store_true", help="Start with the weights specified in config")
...
validation_set_loader = DataLoader(dataset=validation_set, batch_size=BATCH_SIZE//2, num_workers=THREADS, shuffle=False)
...
def train_model(model_name, model, lr=LEARNING_RATE, epochs=EPOCHS, momentum=MOMENTUM, weight_decay=0, train_loader=training_set_loader, test_loader=validation_set_loader):
    ...
    for e in range(epochs):
        if True or (e + 1) % 10 == 0:
            torch.save(model, 'result_weights/epoch_' + str(e) + ".pt")
...
resnet152_model = resnet.resnet50(pretrained=False, **classes)
if args.load:
    resnet152_model = torch.load(WEIGHTS_PATH)
else:
